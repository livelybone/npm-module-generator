...
module.exports = {
  options: (packageJson = {}) => ({
    'project-name': {
      defaultVal: __dirname => packageJson.name || __dirname.split(path.sep).pop(),
      replace: /{{projectName}}/,
    },
    'description': {
    ...
    'bin': {
      defaultVal() {
        return (packageJson.bin && Object.keys(packageJson.bin)[0]) || this['project-name'].value;
      },
      replace: '{{bin}}',
    },
    'repository': {
      defaultVal: () => packageJson.repository.url,
      replace: '{{repository}}',
    },
    'keywords': {
      dealFn(answer) {
        return JSON.stringify((packageJson.keywords) || answer.split(',').map(word => word.trim()));
      },
      replace: '"{{keywords}}"',
    },
    'bugsUrl': {
