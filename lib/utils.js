...
function dealArgs(generatorTypes) {
  ...
  console.log(args1)
  const generatorType = generatorTypes.find(t => args1.includes(t.name))
    || generatorTypes[0]
  console.log(generatorTypes, generatorType)
  return { generatorType, moduleName: args.find(arg => !reg.generatorType.test(arg)) }
}
